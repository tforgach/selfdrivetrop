...
# load datasets
#dsTrain = datasets.load('processedImgs')
#dsTest = datasets.load('processedImgs')
#xTrain, xTest, yTrain, yTest = train_test_split(xTrain, xTrain, test_size=0.2)
#xTrain = [x for x,y in dsTrain]
#xTest = [x for x,y in dsTest]
#yTrain = [y for x,y in Train]
...
xTest, yTest = preprocessData(pathTest, np.zeros(testSize), imgHeight, imgWidth)
